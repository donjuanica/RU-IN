<?php 
    ...
    */
    ...
    class TableEditor
    {
        ...
        */
        function TableEditor($db, $table, $page_title)
        {
            /**
        ...
        */
        function display()
        {
            ...
            */
			...
			$fields = '`'.$fields.'`';
			$sql = "SELECT $fields FROM $tables WHERE $joinClause AND $filters AND ($searchClause) $orderbyClause LIMIT $startOffset,$perPage";
            $results = $this->dbGetAll($s = $sql);
//print_r($s);
            if ($results === false) {
                $this->errors[] = 'Error getting data. Database said: ' . $this->dbError(). ' ---- ' . htmlentities($sql);
            }
                ...
                <div style="float: left; text-align: left">
                    ...
                    <?php if($this->getConfig('allowASearch')):?>
                        <?php if(!empty($_GET['asearch'])):?>
                            Advanced Search: <a href="<?php echo $modifyURL;?>">Modify</a> :: <a href="<?php echo $clearURL;?>">Clear</a>
                        <?php else:?>
                            <a href="<?php echo $aSearchURL;?>">Advanced Search</a>
                        <?php endif;?>
                    ...
                    <?php if(isset($this->search)):?>
                        <a href="<?php echo $clearURL;?>">Clear search</a>
                    <?php endif;?>
                </div>
            <?php endif;?>
            ...
            <tr <?php echo ($k % 2 == 1 ? 'class="altRow"' : '');?> style="cursor: default" onClick="row_highlight(event, this, this.getElementsByTagName('input')[0], '<?php echo ($k % 2 == 1 ? 'altRow' : '');?>')">
                ...
                <?php foreach($row as $field => $v):?>
                    <?php if(empty($this->fields[$field]['noDisplay'])):?>
                        <td valign="top">
						<?php echo $v // Do not do htmlspecialchars here - it's now implemented as a display filter;?></td>
                    <?php endif;?>
                <?php endforeach;?>
...
<head>
    ...
    <!--
		...
		.actionButton, button, .actionButton_classes {
		   color:#666;
		   font-family:Verdana;
		   font-weight:bold;
...
<link rel="icon" type="image/png" href="../Basketball_32x32.png" />
    <script language="javascript" type="text/javascript">
    <!--
        /**
